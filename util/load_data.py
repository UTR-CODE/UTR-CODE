...
class UTRDataset_combinedenv(Dataset):
    def __init__(self, species="human", sample="train", gene="train", dataset_dir="../training_rawdata"):
        ...
        data = pd.read_parquet(os.path.join(dataset_dir, f"{species}_TPM.parquet"))

        env_dir_data = os.path.join(dataset_dir,
                                    f"../VAE_project/1024_6species/finetune/{species}/{species}_compressed.csv")
        ...
        self.data["RPF_TPM"] = np.log1p(self.data["RPF_TPM"] * folds)
