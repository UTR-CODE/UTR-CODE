...
class CNN_Encoder_Model(nn.Module):
    ...
    def __init__(self, model_config, input_shapes):
        ...
        self.RPF_human = nn.Sequential(
            nn.Linear(in_features=cds_flatten_size, out_features=30),#88256 --> 128
            nn.ReLU(),
            nn.BatchNorm1d(30),
        ...
        self.fc_human = nn.Linear(in_features=30, out_features=1)#128 --> 1


        self.attention_utr = nn.Sequential(

            nn.Linear(
                in_features=int(p["dense6"]),#32
                out_features=int(p["filters1"]) * self.motif_depth,#256 * 4
            ),
            nn.Tanh(),
        )
        ...
        self.mRNA_layer = nn.Sequential(
            nn.Linear(
                in_features=self.input_shapes["trans"][0], out_features=int(p["dense4"])#1040 --> 512
            ),
            nn.ReLU(),
            nn.BatchNorm1d(int(p["dense4"])),
            nn.Dropout(float(p["dropout4"])),
            nn.Linear(in_features=int(p["dense4"]), out_features=int(p["dense6"])),#512 --> 32
            nn.ReLU(),
            nn.BatchNorm1d(int(p["dense6"])),
        )
    ...
    def forward(self, regSequence, cdsSequence, transArray, rnaCount):
        ...
        #1 head
        hidden_feature = self.RPF_human(cds_seq_features)#[256,30]
        TE = self.fc_human(hidden_feature)#[256, 1]
        RPF = TE * rnaCount + self.bias
        result = torch.concat([RPF, TE], 1)
        return result
