...
from util.multi_module_vae import CNN_Encoder_Model
import random
import seqpro as sp
...
def load_model(path,weight=None):
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        result = torch.concat(predict_out).cpu().detach().numpy().reshape(-1)
        raw_df["predict_RPF"] = get_orig_rpf(result)
